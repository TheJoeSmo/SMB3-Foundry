...
def create_bytes(pos, domain, index, is_four_bytes) -> bytearray:
...
def test_load_and_save_equal(gen, is_four_bytes, index, domain, x, y, level):
    data = create_bytes((x, y), domain, index, is_four_bytes)
...
def test_changing_pos(gen, is_four_bytes, index, domain, new_x, new_y, x, y, level):
    ...
    assert create_bytes((new_x, new_y), domain, index, is_four_bytes) == obj.to_bytes(False)
...
def test_changing_index(gen, is_four_bytes, index, domain, x, y, level):
    obj = LevelObject.from_bytes(
        FakeObjectSet(gen, is_four_bytes), create_bytes((x, y), domain, index, is_four_bytes), level
    )
    obj.index = (index & 0xF0) + 0x03
    assert create_bytes((x, y), domain, (index & 0xF0) + 0x03, is_four_bytes) == obj.to_bytes(level)
