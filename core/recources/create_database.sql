...
CREATE TABLE FillerUserTypes (
    FUTID INTEGER PRIMARY KEY,
    Name TEXT
);
INSERT INTO FillerUserTypes VALUES (1, "Draw Update")

CREATE TABLE IF NOT EXISTS FillerUsers (
    FUID INTEGER PRIMARY KEY
    FillerID INTEGER REFERENCES Fillers(FillerID),
    FillerUserID INTEGER,
    FUTID INTEGER NOT NULL REFERENCES FillerUserTypes(FUTID),
);
...
CREATE TABLE IF NOT EXISTS DrawUpdates (
    ...
    TopPTID INTEGER NOT NULL REFERENCES PatternTables(PTID),
    BottomPTID INTEGER NOT NULL REFERENCES PatternTables(PTID),
    FUTID AS 1 PERSISTENT,
    FOREIGN KEY (DrawUpdateID, FUTID) REFERENCES FillerUsers (FillerUserID, FUTID)
);
